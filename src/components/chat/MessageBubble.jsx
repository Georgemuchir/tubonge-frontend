      ...
      <div className={`nexus-bubble-modern ${isOwnMessage ? 'bubble-me' : 'bubble-other'}`}>
        ...
        {isImage ? (
          ...
          <div style={{ position: 'relative', display: 'inline-block', marginBottom: 4 }}>
            {videoError ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: 240, height: 120, background: 'rgba(255,255,255,0.05)', borderRadius: 8, color: '#9ca3af', fontSize: 13, gap: 6 }}>
              ...
              <video
                ...
                playsInline
                src={mediaUrl(message.content)}
                style={{ maxWidth: 280, maxHeight: 220, borderRadius: 8, display: 'block', opacity: message.status === 'sending' ? 0.75 : 1, background: '#000' }}
                preload="metadata"
                onError={() => setVideoError(true)}
              />
            )}
            {message.status === 'sending' && !videoError && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(0,0,0,0.35)', pointerEvents: 'none' }}>
                <div className="animate-spin" style={{ width: 30, height: 30, border: '3px solid rgba(255,255,255,0.7)', borderTopColor: 'transparent', borderRadius: '50%' }} />
              </div>
            )}
          </div>
        ) : isVoice ? (
